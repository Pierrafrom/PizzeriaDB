...
SELECT co.orderId,
       ...
       p.pizzaPrice + (IFNULL(pci.additionalCost, 0)) AS price, -- Price calculation
       NULL                                           AS unit
FROM CLIENT_ORDER co
         JOIN
...
SELECT pc.pizzaId              AS CustomPizzaId,
       ...
       pci.quantityRemoved     AS QuantityRemoved,
       i.ingredientQuantity    AS Stock
